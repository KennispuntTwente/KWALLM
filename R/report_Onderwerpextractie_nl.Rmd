...
if (!isTRUE(rl$assign_multiple_categories)) {
  ...
  dplyr::arrange(dplyr::desc(Aantal), result) |>
  ...
  rl$df |>
    ...
    dplyr::arrange(dplyr::desc(Aantal), Onderwerp) |>
    DT::datatable(
      rownames = FALSE,
      extensions = 'Buttons',
      options = list(
...
# Sort paragraphs by category frequency (most frequent first)
if (!is.null(rl$paragraphs) && length(rl$paragraphs) > 0) {
  # Calculate frequency for each category/topic
  if (!isTRUE(rl$assign_multiple_categories)) {
  ...
  # Extract topics from paragraphs and match with frequencies
  paragraph_topics <- sapply(rl$paragraphs, function(p) p$topic)
  
  # Create order based on frequency (descending) then alphabetically (ascending)
  topic_order <- match(paragraph_topics, category_freq$result)
  freq_order <- category_freq$freq[topic_order]
  sorted_indices <- order(freq_order, paragraph_topics, decreasing = c(TRUE, FALSE), method = "radix")
  
  # Reorder paragraphs
  rl$paragraphs <- rl$paragraphs[sorted_indices]
}
